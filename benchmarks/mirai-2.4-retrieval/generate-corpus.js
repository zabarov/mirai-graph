...
for (const [domain, concepts] of Object.entries(domains)) {
  ...
  concepts.forEach(([slug, en, ru, intent], index) => {
    ...
    const qrels = { [documentId]: 3, [`graph.object:${objectId}`]: 2 };
    ...
    queries.push({ ...base, id: `query.${objectId}.same`, language: sourceLanguage, variant: "same_language", query: sourceLanguage === "en" ? en : ru });
