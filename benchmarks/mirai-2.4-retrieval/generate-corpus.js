...
for (const [domain, concepts] of Object.entries(domains)) {
  ...
  concepts.forEach(([slug, en, ru, intent], index) => {
    ...
    else units.push({ contract_version: "1.0.0", id: documentId, source_ref: source, source_fingerprint: sourceDigest(source), kind: "text", media_type: "text/markdown", ordinal: index, content: text, content_digest: digestValue(text), source_span: { section: sourceLanguage === "en" ? en : ru }, authority: "owner_asserted", scope, confidentiality: "public", instructions_authorized: false });
...
units.push({ contract_version: "1.0.0", id: "unit.restricted-control", source_ref: "source.restricted", source_fingerprint: sourceDigest("source.restricted"), kind: "text", media_type: "text/markdown", ordinal: 999, content: "Restricted control must never appear in authorized results.", content_digest: digestValue("restricted"), authority: "owner_asserted", scope: "scope.restricted", confidentiality: "restricted", instructions_authorized: false });
