...
const { createOutcomePureAdapters, outcomeOperationCatalog } = require("../../dist/cjs/outcome");
...
test("existing Mirai Program composes outcome operations without new node kinds", async () => {
  ...
  const adapters = createOutcomePureAdapters(outcomeOperationCatalog().digest);
  const episode = await executePure(program, { contract: load("outcome-contract.json"), candidates: load("candidate-set.json"), evidence: load("evidence-set.json") }, { adapters });
