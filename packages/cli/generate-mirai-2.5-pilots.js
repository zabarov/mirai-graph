...
function slot(id, acquisition = "evidence") { return { id, label: id.replaceAll("_", " "), value_type: "string", critical: true, acquisition, evidence_required: acquisition !== "user_input", minimum_authority: "supporting", freshness_required: "current" }; }
function contract(id, required, options = {}) { return seal({ contract_version: "1.0.0", id, goal: options.goal || id, scope: { purpose: options.purpose || "knowledge_assistance", domains: options.domains || ["support"], effect: "read_only" }, outcome_kind: options.kind || "answer", required_slots: required, optional_slots: [], completion_policy: { allow_partial: true, all_critical_required: true }, interaction_policy: { max_questions: 1, ask_one_at_a_time: true }, conflict_policy: { critical_conflict: "block", noncritical_conflict: "report" }, template_authority: "owner_approved", execution_allowed: false, canonical_write_allowed: false }); }
function evidence(ids, options = {}) { return seal({ contract_version: "1.0.0", snapshot_digest: digestValue([ids, options]), policy_digest: digestValue("pilot-policy"), items: ids.map((id) => ({ id: `evidence.${id}`, source_ref: `source.${id}`, authority: "owner_asserted", freshness: "current", conflict_refs: options.conflict === id ? [`conflict.${id}`] : [], authorized: options.unauthorized !== id, digest: digestValue(id) })), partial: false, limitations: ["Synthetic shadow evidence; no production effectiveness claim."], canonical_write_allowed: false }); }
...
function runCase(id, contractValue, candidateValue, evidenceValue, expected) { const assessment = assessOutcome(contractValue, candidateValue, evidenceValue); const delivery = planOutcomeDelivery(assessment); return { id, expected_status: expected, actual_status: assessment.status, assessment_digest: assessment.digest, evidence_coverage: assessment.evidence_coverage, question_count: assessment.clarifying_question ? 1 : 0, citation_count: delivery.citations.length }; }
...
const allEvidence = evidence(["answer", "route"]);
...
const c07 = runCase("C07_needs_parameter", inputContract, candidates("input", inputContract, ["answer"]), evidence(["answer"]), "needs_input");
const c08 = runCase("C08_conflicting", standard, allCandidates, evidence(["answer", "route"], { conflict: "answer" }), "blocked_by_conflict");
write("knowledge-assistant-result.json", pilot("pilot.knowledge-assistant", [c01, c02, c03, c04, c05, c06, c07, c08], "shadow_no_send"));

const childContract = contract("outcome.project-child", [slot("spec"), slot("tests")], { purpose: "project_delivery", domains: ["project"] });
const childEvidence = evidence(["spec", "tests"]); const childCandidates = candidates("child", childContract, ["spec", "tests"]);
const childA = assessOutcome(childContract, childCandidates, childEvidence); const childB = assessOutcome(childContract, childCandidates, childEvidence);
const aggregate = aggregateOutcomes(childContract, [childA, childB]);
write("federation-project-result.json", pilot("pilot.federation-project", [{ id: "parent_child_aggregation", expected_status: "satisfied", actual_status: aggregate.status, assessment_digest: aggregate.digest, evidence_coverage: aggregate.evidence_coverage, question_count: 0, citation_count: planOutcomeDelivery(aggregate).citations.length }]));
...
const employeeCase = runCase("approval_boundary", employeeContract, candidates("employee", employeeContract, ["decision", "result"]), evidence(["decision", "result"]), "needs_input");
