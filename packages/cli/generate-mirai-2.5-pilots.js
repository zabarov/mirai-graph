...
const aggregate = aggregateOutcomes(parentContract, [childA, childB]);
const incompleteChild = assessOutcome(childContract, candidates("incomplete-child", childContract, ["spec"]), childEvidence);
const incompleteAggregate = aggregateOutcomes(parentContract, [childA, incompleteChild]);
write("federation-project-result.json", pilot("pilot.federation-project", [
  { id: "parent_child_aggregation", expected_status: "satisfied", actual_status: aggregate.status, assessment_digest: aggregate.digest, evidence_coverage: aggregate.evidence_coverage, question_count: 0, citation_count: planOutcomeDelivery(aggregate).citations.length },
  { id: "incomplete_child_blocks_parent", expected_status: "insufficient_evidence", actual_status: incompleteAggregate.status, assessment_digest: incompleteAggregate.digest, evidence_coverage: incompleteAggregate.evidence_coverage, question_count: 0, citation_count: planOutcomeDelivery(incompleteAggregate).citations.length }
]));
