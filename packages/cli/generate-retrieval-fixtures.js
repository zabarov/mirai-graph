...
async function main() {
  ...
  const evaluation = evaluateRetrieval("corpus.mirai-retrieval-minimal", "mirai_planner", [{ expected_document_ids: ["unit.release-policy"], expected_intent: "policy_lookup", predicted_intent: result.plan.intent, hits: result.evidence.hits, answer: result.answer, latency_ms: 5 }]);
