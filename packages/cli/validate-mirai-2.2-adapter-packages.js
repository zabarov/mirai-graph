...
const packages = [
  ...
  ["source-postgres", "@zabarov/mirai-source-postgres", "createPostgresSourceProvider"],
  ["source-mysql", "@zabarov/mirai-source-mysql", "createMysqlSourceProvider"],
  ["source-s3", "@zabarov/mirai-source-s3", "createS3SourceProvider"]
];

for (const [directory, name, factory] of packages) {
  const packageRoot = path.join(root, "packages", directory);
  const manifest = JSON.parse(fs.readFileSync(path.join(packageRoot, "package.json"), "utf8"));
  assert.equal(manifest.name, name);
  assert.equal(manifest.version, rootPackage.version);
  assert.equal(manifest.peerDependencies?.["@zabarov/mirai"], ">=2.2.0-alpha.1 <3");
  ...
  assert.equal(typeof implementation[factory], "function");
}
